...
test('hits retain only evidence cards and receipts while deleting the downloaded source video',async t=>{
 ...
 const update=p.store.update.bind(p.store);let repeatUpdates=0;t.mock.method(p.store,'update',(...args)=>{repeatUpdates++;return update(...args);});await p.syncVerdicts();assert.equal(repeatUpdates,0,'a settled hit is not rewritten or pruned again');
});
