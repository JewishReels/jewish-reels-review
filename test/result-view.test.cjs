...
const hit={id:'fho-1',source_key:'source-a',verdict:'jewish',title:'A title',cues:['judaica'],summary:'Visible ritual object.',confidence:.9,cards_reviewed:['a.jpg','b.jpg'],evidence:{card_path:'frames/fho-1/cards/a.jpg',primary:{evidence:'x'.repeat(5000)}}};
...
test('result summaries retain navigation fields and omit nested evidence and coverage arrays',()=>{
 ...
 assert.equal(values[0].source_key,'source-a');assert.deepEqual(values[0].source_keys,['source-a']);assert.equal(values[0].source_count,1);
...
test('copied hit chains appear once while retaining every catalog id',()=>{
 ...
 const first={...original,id:'copy-1',source_key:'source-b',copied_from:original.id},second={...original,id:'copy-2',source_key:'source-b',copied_from:first.id};
 ...
 assert.deepEqual(values[0].source_keys,['source-a','source-b']);assert.equal(values[0].source_count,2);
