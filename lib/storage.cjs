...
function decodeLedger(raw) {
  ...
  if (raw && typeof raw === 'object') {
    ...
    if (pairs.every(([, v]) => v && typeof v === 'object' && !Array.isArray(v) && typeof v.verdict === 'string')) {
      return { entries: pairs.map(([k, v]) => ({ ...v, id: v.id ?? k })), encode: entries => Object.fromEntries(entries.map(v => [String(v.id), v])) };
    }
  }
...
async function discover(selected, { skipIds = new Set(), sourceKey = null } = {}) {
  ...
  const entries = sourceKey ? ledger.entries.filter(e => e.source_key === sourceKey) : ledger.entries;
  ...
  for (const d of dirs) {
    ...
    const m = prepared || meta.get(d.name) || {};
    if (sourceKey && m.source_key !== sourceKey) continue;
    ...
    videos.push({ prepared: prepared || null, id: d.name, cards: names.map(name => path.join(cardsDir, name)), title: String(m.title || ''), url: String(m.url || ''), mp4: typeof m.mp4 === 'string' ? path.resolve(root, m.mp4) : null, expectedFingerprint: managed ? m.source_fingerprint : null, sharedFrom: m.shared_from, sharedFingerprint: m.shared_from ? m.source_fingerprint : null, mediaUrl: m.media_url || '', scope: m.scope, segment_start: m.segment_start, segment_end: m.segment_end });
      ...
      if (!judged.has(d.name)) issues.push({ id: d.name, message: e.message, code: e.code || 'INPUT_UNAVAILABLE', path: e.path });
    }
  }
  return { root, videos, entries, issues, recoveries: recoveries.filter(r => videos.some(v => v.id === r.id)) };
}
async function appendVerdict(root, entry) {
  const p = path.join(root, 'chat_verdicts.json');
  const ledger = await loadLedger(root);
  const found = ledger.entries.find(e => String(e.id) === String(entry.id));
  ...
  await atomicJson(p, ledger.encode(ledger.entries));
...
module.exports = { natural, sha, exists, readJson, atomicJson, loadLedger, discover, appendVerdict, acquireLock, hashFile, fingerprintVideo, fingerprintCards, decodeLedger, mediaFingerprint };
