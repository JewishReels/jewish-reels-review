...
class LearningEngine extends EventEmitter {
 ...
 async run({root,key,model,budget=1,includeNotes=false,beforeStart=async()=>{},applyRules=async()=>{},policy}){
  ...
  finally{if(this.state.status==='waiting'||this.state.status==='pausing')this.update({status:'paused'});this.running=false;await persist().catch(e=>this.update({status:'error',message:`Could not save learning status: ${e.message}`}));this.emit('finished',this.snapshot());}
