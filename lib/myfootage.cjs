...
const CATALOG_PHOTOGRAPHER = '001';
const CATALOG_FORMATS = Object.freeze(['0', '1', '2', '3']);
...
function parseResultsPage(html, pageUrl = `${BASE}results.asp`) {
  ...
  const items = [], resultIds = [], skipped = [];
  for (const raw of rawItems) {
    const id = String(raw?.ItemID || '');
    if (!/^\d+$/.test(id)) throw new Error('MyFootage returned a catalog row without a numeric item ID.');
    resultIds.push(id);
    if (String(raw?.MediaType || 'Video').toLowerCase() !== 'video') {
      skipped.push({ id, reason: 'not-video', media_type: String(raw?.MediaType || '') });
      continue;
    }
    const mediaUrl = safeMediaUrl(raw.PRPath, pageUrl, id);
    if (!mediaUrl) {
      skipped.push({ id, reason: 'no-public-video-preview', media_type: String(raw?.MediaType || 'Video') });
      continue;
    }
    items.push({
      id,
  ...
  return { tallies, items, resultIds, skipped };
}
...
function fullCatalogSeeds() {
  // MyFootage caps an unpartitioned photographer search at 10,000 rows even
  // when more records exist. The site's Refine Search UI defines these four
  // exhaustive, mutually exclusive Contemporary/Archival + Color/B&W format
  // values. Crawling each partition avoids the cap and includes records whose
  // metadata does not place them in one of the homepage decade searches.
  return CATALOG_FORMATS.map(format => `${BASE}results.asp?fotid=${CATALOG_PHOTOGRAPHER}&lstformats=${format}`);
}
...
async function writeCatalog(output, startUrl, rows, authorizedAt, summary = {}) {
  const payload = {
    version: 1,
...
async function crawlAuthorizedCatalog({ startUrl, output, authorized = false, fetchImpl = fetch, signal, onProgress = () => {}, delayMs = 500 }) {
  ...
  let seeds, partitionedCatalog = false;
  if (/^\/results\.asp$/i.test(start.pathname)) seeds = [start.href];
  else if (/^\/(?:index\.asp)?$/i.test(start.pathname)) {
    // Read the selected origin first so an unavailable or replaced site fails
    // before catalog requests begin. Homepage decade cards are not exhaustive:
    // valid public clips can have no decade facet. The official four format
    // partitions cover the whole MyFootage photographer catalog without
    // hitting the server's 10,000-result ceiling.
    await fetchText(start.href, fetchImpl, signal, createCookieJar());
    seeds = fullCatalogSeeds();
    partitionedCatalog = true;
  } else throw new Error('For an authorized MyFootage crawl, use the home page or a results.asp search URL.');

  let completed = 0, expected = seeds.length, declaredCatalogTotal = 0, skippedCatalogRows = 0;
  const partitionOwner = new Map();
  ...
  for (let seedIndex = 0; seedIndex < seeds.length; seedIndex++) {
    ...
    let tallies = null, pageCount = 1, declaredSeedTotal = null;
    for (let pageNumber = 1; pageNumber <= pageCount; pageNumber++) {
      ...
      if (pageNumber === 1) {
        ...
        declaredSeedTotal = Number(parsed.tallies.NumPix);
        if (!Number.isInteger(declaredSeedTotal) || declaredSeedTotal < 0) throw new Error('MyFootage did not declare a valid result count for this catalog partition.');
        declaredCatalogTotal += declaredSeedTotal;
        expected += pageCount - 1;
      } else if (Number(parsed.tallies.NumPix) !== declaredSeedTotal) {
        throw new Error('MyFootage changed the declared result count while a catalog partition was being read.');
      }
      ...
      const pageIds = parsed.resultIds;
      ...
      for (const id of parsed.resultIds) {
        if (seedResultIds.has(id)) throw new Error('MyFootage repeated a catalog row within the same partition.');
        seedResultIds.add(id);
        if (partitionedCatalog) {
          const owner = partitionOwner.get(id);
          if (owner !== undefined && owner !== seedIndex) throw new Error('MyFootage returned the same catalog row in multiple supposedly exclusive partitions.');
          partitionOwner.set(id, seedIndex);
        }
      }
      skippedCatalogRows += parsed.skipped.length;
      ...
      await writeCatalog(output, start.href, [...rows.values()].sort((a, b) => Number(a.id) - Number(b.id)), authorizedAt, {
        catalog_results: declaredCatalogTotal,
        skipped_nonvideo_or_unplayable: skippedCatalogRows
      });
      onProgress({ stage: 'pages', completed, total: expected, reels: rows.size });
    }
    if (seedResultIds.size !== declaredSeedTotal) {
      throw new Error(`MyFootage declared ${declaredSeedTotal} results for a catalog partition, but ${seedResultIds.size} unique catalog rows were received.`);
    }
  }
  const items = [...rows.values()].sort((a, b) => Number(a.id) - Number(b.id));
  if (partitionedCatalog && partitionOwner.size !== declaredCatalogTotal) {
    throw new Error(`MyFootage declared ${declaredCatalogTotal} catalog records, but ${partitionOwner.size} unique catalog rows were received.`);
  }
  if (partitionedCatalog && items.length + skippedCatalogRows !== declaredCatalogTotal) {
    throw new Error(`MyFootage returned ${declaredCatalogTotal} catalog rows, but only ${items.length} playable videos and ${skippedCatalogRows} explicitly skipped rows were accounted for.`);
  }
  await writeCatalog(output, start.href, items, authorizedAt, {
    catalog_results: declaredCatalogTotal,
    skipped_nonvideo_or_unplayable: skippedCatalogRows
  });
  return { file: output, total: items.length, catalogTotal: declaredCatalogTotal, skipped: skippedCatalogRows, pages: completed, sourceKey: 'myfootage', label: 'MyFootage' };
}
