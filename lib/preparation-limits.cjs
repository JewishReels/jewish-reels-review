...
function preparationLimits(load = {}, maxGB = 20, parallelism = os.availableParallelism?.() || os.cpus().length) {
  // Review can span 128 videos, while preparation keeps its own conservative
  // source and FFmpeg ceilings below. This lets downloads overlap without
  // multiplying expensive video decoders to match the model-request pool.
  const requestedSources = Math.min(128, positiveInteger(load.videoConcurrency, 1));
