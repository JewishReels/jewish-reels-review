...
function stableId(url) {
  ...
  if (M.itemIdFromUrl(url)) return `mf-${M.itemIdFromUrl(url)}`;
...
class QueueStore {
  ...
  import(file, options = {}) {
    ...
    const validSourceUrls=rows.map(row=>webUrl(row.url)).filter(Boolean);
    const onlyMyFootage = validSourceUrls.length > 0 && validSourceUrls.every(url => M.isMyFootageUrl(url));
    const key = options.sourceKey || (onlyMyFootage ? 'myfootage' : sourceKey(file));
    const label = options.label || (key === 'myfootage' ? 'MyFootage' : path.basename(file));
    const kind = ['crawled','website'].includes(options.kind) ? options.kind : 'imported';
    ...
    try {
      ...
      for (const r of rows) {
        const rawUrl = webUrl(r.url), url = rawUrl && (M.canonicalItemUrl(rawUrl) || rawUrl); if (!url) { invalid++; continue; }
        const catalogId=String(r.catalog_id??''),title=String(r.title || ''),now=new Date().toISOString();
        const desiredId=key==='footagefarm'&&/^\d+$/.test(catalogId)?`ff-${catalogId}`:key==='myfootage'&&/^\d+$/.test(catalogId)?`mf-${catalogId}`:stableId(url);
        ...
        if (!item) {
          ...
          if (emptyShell) {
            ...
            if (occupied) id=`${key==='footagefarm'?'ff':key==='myfootage'?'mf':'url'}-url-${sha(url).slice(0,20)}`;
  ...
  registerSource({ sourceKey: key, label, homepage, kind = 'website' }) {
    if (!/^[a-z0-9][a-z0-9-]{0,80}$/.test(String(key || ''))) throw new Error('Source key is invalid.');
    const url=webUrl(homepage);if(!url)throw new Error('Source homepage must be a public HTTP(S) URL.');
    const file=`website:${url}`;
    this.db.prepare('INSERT OR REPLACE INTO imports(file,at,added,source_key,label,kind) VALUES(?,?,?,?,?,?)')
      .run(file,new Date().toISOString(),0,key,String(label||key),kind);
    return {sourceKey:key,label:String(label||key),homepage:url,total:0,added:0,alreadyPresent:0,file};
  }
  ...
  sourcesList() { return this.db.prepare("SELECT imports.source_key key,COALESCE(MAX(NULLIF(imports.label,'')),imports.source_key) label,CASE MAX(CASE imports.kind WHEN 'crawled' THEN 3 WHEN 'website' THEN 2 ELSE 1 END) WHEN 3 THEN 'crawled' WHEN 2 THEN 'website' ELSE 'imported' END kind,MAX(imports.at) imported_at,COUNT(DISTINCT item_sources.item_id) total FROM imports LEFT JOIN item_sources USING(source_key) WHERE imports.source_key IS NOT NULL AND imports.source_key<>'' GROUP BY imports.source_key ORDER BY label").all(); }
  ...
  retryVimeoAuthenticationRequired() {
    // Keep the exact auth error as durable provenance. The pipeline can then
    // reuse the already verified official Vimeo URL instead of performing the
    // same rate-limited profile search after access is configured.
    return Number(this.db.prepare(`UPDATE items SET status='pending',updated_at=? WHERE status='error'
      AND error LIKE 'Vimeo access is required for this Footage Farm screener.%'`).run(new Date().toISOString()).changes);
  }
  retryScrapflyRequired() {
    return Number(this.db.prepare(`UPDATE items SET status='pending',error=NULL,updated_at=? WHERE status='error' AND (
      error LIKE 'Scrapfly access is required while Vimeo is rate-limiting the public Footage Farm catalog lookup.%'
      OR error LIKE 'Source retry: Source HTTP 429 (vimeo.com).%'
