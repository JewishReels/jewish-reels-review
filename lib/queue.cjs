...
class QueueStore {
  ...
  import(file, options = {}) {
    const rows = sourceRows(file); let added = 0, invalid = 0;
    ...
    const insert = this.db.prepare("INSERT OR IGNORE INTO items(id,url,title,source_file,catalog_id,updated_at) VALUES(?,?,?,?,?,?)");
    const attach = this.db.prepare('INSERT OR IGNORE INTO item_sources(item_id,source_key) VALUES(?,?)');
    this.db.exec('BEGIN IMMEDIATE');
    try {
      for (const r of rows) { const url = webUrl(r.url); if (!url) { invalid++; continue; } const catalogId=String(r.catalog_id??''),id=key==='footagefarm'&&/^\d+$/.test(catalogId)?`ff-${catalogId}`:stableId(url); added += Number(insert.run(id, url, String(r.title || ''), file, catalogId, new Date().toISOString()).changes); attach.run(id,key); }
    ...
    return { added, invalid, alreadyPresent: rows.length - added - invalid, total: this.counts().total, file, sourceKey:key, label };
  }
  ...
  update(id, changes) {
    ...
    this.db.prepare(`UPDATE items SET ${pairs.map(([k]) => `${k}=?`).join(',')},updated_at=? WHERE id=?`).run(...pairs.map(([, v]) => v ?? null), new Date().toISOString(), id);
  }
