  ...
  for (const item of await R.retryIO(() => io.readdir(folder, { withFileTypes: true })).catch(e => { if (e.code === 'ENOENT') return []; throw e; })) {
    ...
    if (item.isDirectory()) bytes += await treeBytes(file, io, seenFiles, options);
      ...
      if (stat.isFile()) {
        ...
        if (!options.lastLinkOnly || links <= 1) bytes += Number(stat.size);
      }
...
class PreparationPipeline extends EventEmitter {
  constructor({ tools, media = M, frameFilter, getFrameFilter = () => ({enabled:false}), getReviewLoad = () => ({}) }) {
    ...
    this.state = { status: 'idle', message: 'Import a URL collection to begin.', current: null, counts: {}, recent: [], bytes: 0, footageBytes: 0, cardBytes: 0, reclaimed: 0 };
    this.judgedIds = new Set(); this.deferredIds = new Set(); this.manualDeferredIds = new Set(); this.cleanupRetries = new Map(); this.preparationRetries = new Map(); this.reviewId = null; this.reviewIds = new Set(); this.target = 3;
  ...
  async open(root) {
    ...
    this.store?.close(); this.root = root; this.judgedIds = new Set(); this.deferredIds = new Set(); this.manualDeferredIds = new Set(); this.cleanupRetries.clear(); this.preparationRetries.clear(); this.reviewId = null; this.reviewIds = new Set();
  ...
  async usage() {
    ...
    const cardBytes = await treeBytes(path.join(this.root, 'frames'), fs, seenFiles);
    // The storage guard still counts both categories, but the UI distinguishes
    // temporary source footage from the evidence cards the user asked to keep.
    const bytes = footageBytes + cardBytes;
    const stat = await fs.statfs(this.root); return { bytes, footageBytes, cardBytes, free: Number(stat.bavail) * Number(stat.bsize) };
  }
  ...
  async _cleanupNow() {
    ...
    await this.audit({ event: 'manual-media-cleanup', reclaimed_bytes: reclaimed, bytes_remaining: usage.bytes });
    this.update({
      bytes: usage.bytes, footageBytes: usage.footageBytes, cardBytes: usage.cardBytes,
      message: reclaimed
      ...
      while (!this.stopping) {
        ...
        if (diskError) {
          ...
          if (diskError.storageLimit) {
            ...
            if (lastUsage.bytes + admissionReserve < opts.maxGB * GB && lastUsage.free >= 3 * GB) {
              await this.audit({ event: 'storage-guard-resumed', bytes: lastUsage.bytes, reserve_bytes: admissionReserve });
              diskError = null; this.controller = new AbortController(); continue;
            }
            this.update({ status: 'running', current: null, bytes: lastUsage.bytes, footageBytes: lastUsage.footageBytes, cardBytes: lastUsage.cardBytes, message: 'Storage guard reached the configured limit. Clean up media removes temporary footage; saved cards are retained. It will resume automatically when space is available.' });
        ...
        this.update({ bytes: usage.bytes, footageBytes: usage.footageBytes, cardBytes: usage.cardBytes });
        if (usage.bytes + admissionReserve >= opts.maxGB * GB || usage.free < 3 * GB) {diskError=storageError();this.controller.abort();continue;}
...
module.exports = { PreparationPipeline, ownedRemove, treeBytes, admissionReserveBytes };
