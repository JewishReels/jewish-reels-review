...
class PreparationPipeline extends EventEmitter {
  ...
  async acquireSource(row, opts, retry = null) {
    ...
    try {
      ...
      const file = await this.media.download(resolved, folder, this.tools, { signal, maxBytes: Math.min(8 * GB, opts.maxGB * GB / 2), onCandidate: candidate => { downloadedUrl = candidate; }, onRendition: candidate => { downloadedRendition = candidate; }, onProgress: p => { if (!this.extractingId) this.progress({ current: { id: row.id, stage: 'downloading', ...p } }); } });
