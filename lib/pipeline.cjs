...
const {preparationLimits}=require('./preparation-limits.cjs');
const R = require('./recovery.cjs');
const GB = 1024 ** 3;
const LEGACY_FOOTAGE_FARM_MIGRATION = 'footagefarm-related-vimeo-v1';
function isBareVimeoPlayer(value) {
  try {
    const url=new URL(String(value||''));
    return url.protocol==='https:'&&url.hostname.toLowerCase()==='player.vimeo.com'&&/^\/video\/\d+\/?$/.test(url.pathname)&&!url.search;
  } catch { return false; }
}
function savedOfficialFootageFarmVimeo(row) {
  if (!/^Vimeo access is required for this Footage Farm screener\./.test(String(row?.error || ''))) return null;
  try {
    const source = new URL(String(row.url || '')), media = new URL(String(row.media_url || ''));
    if (source.protocol !== 'https:' || !/^(?:www\.)?footagefarm\.com$/i.test(source.hostname) || !/^\/reel-details\//i.test(source.pathname)) return null;
    if (media.protocol !== 'https:' || !/^(?:www\.)?vimeo\.com$/i.test(media.hostname) || !/^\/\d+\/?$/.test(media.pathname) || media.search || media.hash) return null;
    if (row.media_key !== S.sha(media.href)) return null;
    return { provider: 'footagefarm-vimeo', url: media.href, referer: source.href, direct: false, scope: 'whole-reel' };
  } catch { return null; }
}
...
const admissionReserveBytes = concurrency => (Math.max(1, Math.min(12, Number(concurrency) || 1)) + 1) * 256 * 1024 * 1024;
...
function concurrencyGate(limit) {
  ...
  const drain = () => {
    while (active < limit && waiters.length) {
      ...
      active++;
      let released = false;
      waiter.resolve(() => { if (released) return; released = true; active--; drain(); });
    }
  };
  return {
    acquire(signal) {
      ...
      return new Promise((resolve, reject) => {
        ...
        waiter.abort = () => { const index = waiters.indexOf(waiter); if (index >= 0) waiters.splice(index, 1); reject(abortError()); };
...
async function stableSourceHash(file, attempts = 3) {
  for (let attempt = 0; attempt < attempts; attempt++) {
    const before = await sourceIdentity(file);
    const hash = await S.hashFile(file);
    const after = await sourceIdentity(file);
    if (sameSourceIdentity(before, after)) return { hash, identity: after, bytes: Number(after.size) };
  }
  const error = Object.assign(new Error('Downloaded source changed while its identity was being verified. It will be retried from the saved source.'), { code: 'SOURCE_CHANGED', sourceTransient: true });
  throw error;
}
...
class PreparationPipeline extends EventEmitter {
  constructor({ tools, media = M, frameFilter, getFrameFilter = () => ({enabled:false}), getReviewLoad = () => ({}) }) {
    ...
    this.getReviewLoad=getReviewLoad;this.parallelism=undefined;this.minimumBuffer=3;
    ...
    this.wakeWaiters = new Set(); this.sourceClaims = new Map(); this.mediaClaims = new Map(); this.contentClaims = new Map(); this.resolvedAhead = null; this.extractingId = null; this.extractingIds = new Set();
  }
  backlog() {
  ...
  async migrateLegacyFootageFarmMedia() {
    if (this.store.migration(LEGACY_FOOTAGE_FARM_MIGRATION)) return { migrated: 0, verdicts: 0 };
    const planPath=path.join(this.root,'.pipeline','migrations',`${LEGACY_FOOTAGE_FARM_MIGRATION}.json`);
    let plan=await S.readJson(planPath,null);
    if(!plan){
      const rows=this.store.rows(),ids=new Set(rows.filter(row=>/^ff-/.test(row.id)&&isBareVimeoPlayer(row.media_url)).map(row=>row.id));
      const grow=()=>{
        let changed=false;
        const selected=rows.filter(row=>ids.has(row.id)),owners=new Set(selected.map(row=>row.owner_id).filter(Boolean)),hashes=new Set(selected.map(row=>row.source_hash).filter(Boolean)),keys=new Set(selected.map(row=>row.media_key).filter(Boolean));
        for(const row of rows){
          if(ids.has(row.id))continue;
          if((row.owner_id&&ids.has(row.owner_id))||(row.id&&owners.has(row.id))||(row.source_hash&&hashes.has(row.source_hash))||(row.media_key&&keys.has(row.media_key))){ids.add(row.id);changed=true;}
        }
        return changed;
      };
      while(grow()){}
      const ledger=await S.loadLedger(this.root),active=S.activeEntries(ledger.entries),badFingerprints=new Set(active.filter(entry=>ids.has(String(entry.id))).map(entry=>entry.source_fingerprint).filter(Boolean));
      const verdictIds=new Set(active.filter(entry=>ids.has(String(entry.id))||(entry.copied_from&&ids.has(String(entry.copied_from)))||(entry.source_fingerprint&&badFingerprints.has(entry.source_fingerprint))).map(entry=>String(entry.id)));
      for(const id of verdictIds)ids.add(id);
      while(grow()){}
      plan={version:1,migration:LEGACY_FOOTAGE_FARM_MIGRATION,created_at:new Date().toISOString(),ids:[...ids].sort(),verdict_ids:[...verdictIds].sort(),seed_count:rows.filter(row=>/^ff-/.test(row.id)&&isBareVimeoPlayer(row.media_url)).length,status:'planned'};
      await S.atomicJson(planPath,plan);
    }
    const quarantine=path.join(this.root,'.pipeline','quarantine',LEGACY_FOOTAGE_FARM_MIGRATION);
    let moved=0;
    const move=async relative=>{
      const source=path.join(this.root,relative),target=path.join(quarantine,relative);
      if(!await S.exists(source))return;
      if(await S.exists(target))throw new Error(`Both the active and quarantined copies of ${relative} exist. Nothing was deleted; inspect them before retrying.`);
      await fs.mkdir(path.dirname(target),{recursive:true});await fs.rename(source,target);moved++;
    };
    for(const id of plan.ids){
      if(!/^[a-z0-9-]+$/i.test(id))throw new Error('Legacy Footage Farm migration found an invalid queue ID. No queue rows were reset.');
      await move(path.join('frames',id));
      await move(path.join('.pipeline','receipts',`${id}.json`));
      await move(path.join('.pipeline','media',id));
      await move(path.join('.pipeline','staging',id));
    }
    const superseded=await S.supersedeVerdicts(this.root,plan.verdict_ids,'The old Footage Farm resolver associated this catalog row with an unverified related Vimeo player.',{migration:LEGACY_FOOTAGE_FARM_MIGRATION});
    const migrated=this.store.resetMediaRows(plan.ids);
    plan={...plan,status:'complete',completed_at:new Date().toISOString(),moved_paths:moved,reset_rows:migrated,superseded_verdicts:superseded.changed};
    await S.atomicJson(planPath,plan);
    this.store.recordMigration(LEGACY_FOOTAGE_FARM_MIGRATION,{...plan,ids:undefined,verdict_ids:undefined});
    if(plan.ids.length)await this.audit({event:'legacy-footage-farm-media-quarantined',migration:LEGACY_FOOTAGE_FARM_MIGRATION,seed_rows:plan.seed_count,reset_rows:migrated,superseded_verdicts:superseded.changed,moved_paths:moved,quarantine:path.relative(this.root,quarantine)});
    return {migrated,superseded:superseded.changed,moved,seedCount:plan.seed_count};
  }
  async open(root) {
    ...
    this.wakeWaiters.clear(); this.sourceClaims = new Map(); this.mediaClaims = new Map(); this.contentClaims = new Map(); this.resolvedAhead = null; this.extractingId = null; this.extractingIds = new Set();
    ...
    const legacy=await this.migrateLegacyFootageFarmMedia();
    const vimeoRetries=this.store.requeueFootageFarmUnavailable();
    const unavailable=this.store.classifyPreviewUnavailable();
    const footageRetries=this.store.retryLegacyFootageFarmFailures();
    this.update({ status: 'idle', message: legacy.migrated?`${legacy.migrated} legacy Footage Farm rows were safely quarantined and queued with the corrected resolver.`:vimeoRetries?`${vimeoRetries} Footage Farm records were queued for the verified Vimeo recovery pass.`:footageRetries?`${footageRetries} Footage Farm downloads queued for the improved resolver.`:unavailable?`${unavailable} catalog records separated because Footage Farm requires a requested preview.`:'Footage queue ready.', current: null, ...await this.usage() });
  }
  ...
  registerSource(options) { if (!this.store) throw new Error('Create or choose a workspace first.'); if (this.running) throw new Error('Pause preparation before adding a source.'); const result=this.store.registerSource(options);this.store.setActiveSource(result.sourceKey);this.update({activeSource:result.sourceKey,sources:this.store.sourcesList(),message:`${result.label} was added as an empty source. No website request was made.`});return result; }
  ...
  async _retirePublishedSource(row, reason = 'cards-published', trackReclaimed = true, verifiedSource = null) {
    const current = this.store.get(row.id) || row;
    const key = `published:${current.id}`;
    if ((this.cleanupRetries.get(key)?.next || 0) > Date.now()) return 0;
    // URL aliases normally never download. Content aliases are discovered only
    // after both signed URLs have downloaded and been hashed, so retire that
    // now-redundant copy while retaining the tiny shared descriptor.
    if (current.owner_id && current.owner_id !== current.id) {
      try {
        const targets = [`.pipeline/media/${current.id}`, `.pipeline/staging/${current.id}`];
        let reclaimed = 0;
        for (const relative of targets) reclaimed += await ownedRemove(this.root, relative, this.token);
        this.store.update(current.id, { bytes: 0 });
        if (reclaimed) {
          if (trackReclaimed) this.state.reclaimed += reclaimed;
          await this.audit({ event: 'retired-shared-source', id: current.id, owner_id: current.owner_id, reason, reclaimed_bytes: reclaimed, targets });
        }
        this.cleanupRetries.delete(key);
        return reclaimed;
      } catch (e) {
    ...
    try {
      ...
      if (mp4 && await S.exists(mp4)) {
        // The source was already read in full to compute its SHA-256 before
        // extraction. If the exact owned file identity and timestamps are still
        // unchanged, hashing the entire MP4 again here only keeps a preparation
        // worker busy and creates avoidable disk contention. Restarts and any
        // changed file still take the conservative full-hash path.
        const unchanged = verifiedSource
          && path.resolve(verifiedSource.file) === mp4
          && verifiedSource.fingerprint === receipt.source_fingerprint
          && sameSourceIdentity(verifiedSource.identity, await sourceIdentity(mp4));
        if (!unchanged) {
  ...
  async _syncVerdicts() {
    ...
    // Publication cleanup can lose a race with an antivirus/indexer lock. Retry
    // it during ordinary maintenance so a transient lock cannot leave large
    // source files in place until the storage guard is already full.
    for (const [key, retry] of [...this.cleanupRetries]) {
      if (!key.startsWith('published:') || (retry?.next || 0) > Date.now()) continue;
      const id = key.slice('published:'.length), row = this.store.get(id);
      if (!row) { this.cleanupRetries.delete(key); continue; }
      await this._retirePublishedSource(row, 'publication-cleanup-retry');
    }
    ...
    for (const row of [...this.store.all('ready'), ...this.store.all('no'), ...this.store.all('filtered_no'), ...this.store.all('hit')]) {
      ...
      // `hit` is the durable completion marker for source retirement and
      // evidence-card pruning. Reprocessing every settled hit turned one new
      // verdict into hundreds of FULL-synchronous SQLite writes and filesystem
      // scans. Rows with an error or remaining source bytes still take the
      // recovery path below.
      if (verdict.verdict === 'jewish' && row.status === 'hit' && !row.error && !Number(row.bytes || 0) && !this.cleanupRetries.has(row.id)) continue;
  ...
  async publish(row, receipt, verifiedSource = null, onPublished = null) {
    ...
    if (await S.exists(final)) {
      ...
      if (old?.token === this.token && old.source_fingerprint === receipt.source_fingerprint) { await this.screenPrepared(row,old,final); this.store.update(row.id, { status: 'ready',error:null }); onPublished?.(); await this._retirePublishedSource(this.store.get(row.id), 'cards-republished', true, verifiedSource); this.emit('ready',{id:row.id}); return; }
    ...
    // Exact-content waiters need only the durable receipt, atomically published
    // frames, and ready owner row. Let them reuse the owner while its card
    // signature and temporary-source cleanup finish in this worker.
    onPublished?.();
    await this._retirePublishedSource(this.store.get(row.id), 'cards-published', true, verifiedSource);
  ...
  prefetchResolve(exceptId) {
    ...
    const saved = savedOfficialFootageFarmVimeo(next);
    const promise = saved ? Promise.resolve(saved) : this.media.resolveMedia(next.url, this.controller.signal, this.tools.resolverFetch);
  ...
  async resolveRow(row, signal) {
    ...
    const saved = savedOfficialFootageFarmVimeo(row);
    return saved || this.media.resolveMedia(row.url, signal, this.tools.resolverFetch);
  }
  ...
  async publishAlias(row, resolved, mediaKey) {
    ...
    if (!source || source.token !== this.token || source.id !== shared.id || source.shared_from) throw new Error('Shared-reel owner receipt missing or inconsistent. Resolve its preparation first.');
  ...
  async publishContentAlias(row, resolved, hash) {
    const fingerprint = S.mediaFingerprint(hash, resolved);
    for (const shared of this.store.sameContent(hash, row.id)) {
      const source = await S.readJson(path.join(this.root, '.pipeline', 'receipts', `${shared.id}.json`), null);
      // The byte hash alone identifies a reel, not a story within it. Reuse only
      // an exact source fingerprint, which includes segment boundaries.
      if (!source || source.token !== this.token || source.id !== shared.id || source.shared_from || source.source_fingerprint !== fingerprint) continue;
      this.store.update(row.id, { owner_id: shared.id, source_hash: hash });
      await this.publish(row, { ...source, id: row.id, title: row.title, url: row.url, source_key: this.store.activeSource, media_url: resolved.url, shared_from: shared.id, shared_basis: 'identical downloaded source bytes and story range', scope: resolved.scope || 'whole-reel', segment_start: resolved.segment_start, segment_end: resolved.segment_end });
      await this.audit({ event: 'source-content-reused', id: row.id, from: shared.id, source_fingerprint: fingerprint });
      return true;
    }
    return false;
  }
  async acquireSource(row, opts, retry = null) {
    ...
    this.store.update(row.id, { status: 'resolving', ...(retry ? {} : { attempts: row.attempts + 1 }), error: null });
    if (!this.extractingId) this.update({ current: { id: row.id, stage: 'resolving' }, message: `Resolving ${row.id}…` });
    let resolved = retry?.resolved || await this.resolveRow(row, signal);
    const mediaKey = retry?.mediaKey || (resolved.scope === 'segment' ? 'segment:' + S.sha(JSON.stringify([resolved.url, resolved.segment_start, resolved.segment_end])) : S.sha(resolved.url));
    ...
    let releaseSource=()=>{};
    try {
      const sourceClaimUrl = resolved.url;
      while (this.sourceClaims.has(sourceClaimUrl)) {
        await this.sourceClaims.get(sourceClaimUrl);
        if (this.stopping) throw new Error('Preparation paused. Partial work is saved.');
        if (await this.publishAlias(row, resolved, mediaKey)) { releaseMedia(); return null; }
      }
      let settle;
      const pending = new Promise(resolve => { settle = resolve; });
      this.sourceClaims.set(sourceClaimUrl, pending);
      releaseSource = () => {
        if (this.sourceClaims.get(sourceClaimUrl) === pending) this.sourceClaims.delete(sourceClaimUrl);
        settle();
      };
      ...
      this.prefetchResolve(row.id);
      let downloadedUrl = resolved.url, downloadedRendition = null;
      const file = await this.media.download(resolved, folder, this.tools, { signal, maxBytes: Math.min(8 * GB, opts.maxGB * GB / 2), onCandidate: candidate => { downloadedUrl = candidate; }, onRendition: candidate => { downloadedRendition = candidate; }, onResolved: candidate => {
        if (candidate?.provider === 'footagefarm-vimeo' && /^https:\/\/(?:www\.)?vimeo\.com\/\d+\/?$/i.test(String(candidate.url || ''))) {
          this.store.update(row.id, { media_url: candidate.url, media_key: S.sha(candidate.url) });
        }
      }, onProgress: p => { if (!this.extractingId) this.progress({ current: { id: row.id, stage: 'downloading', ...p } }); } });
      if (downloadedRendition) resolved = downloadedRendition;
      else if (downloadedUrl !== resolved.url) {
        const alternates = resolved.alternates || [], selected = alternates.findIndex(candidate => (typeof candidate === 'string' ? candidate : candidate.url) === downloadedUrl);
        const selectedRendition = selected >= 0 ? alternates[selected] : null, base = { ...resolved, alternates: [] };
        resolved = { ...base, ...(typeof selectedRendition === 'object' ? selectedRendition : {}), url: downloadedUrl, alternates: selected >= 0 ? alternates.slice(selected + 1) : [] };
      }
      this.store.update(row.id, { media_url: resolved.url });
      // Expose a verified source to other stories from this reel before extraction.
      const verified = await stableSourceHash(file);
      const {hash,bytes}=verified;
      const verifiedSource = { file: path.resolve(file), identity: verified.identity, fingerprint: S.mediaFingerprint(hash, resolved) };
      this.store.update(row.id,{source_hash:hash,bytes});releaseSource();
      const fingerprint = S.mediaFingerprint(hash, resolved);
      if (await this.publishContentAlias(row, resolved, hash)) { releaseMedia(); return null; }
      while (this.contentClaims.has(fingerprint)) {
        await this.contentClaims.get(fingerprint);
        if (this.stopping) throw new Error('Preparation paused. Partial work is saved.');
        if (await this.publishContentAlias(row, resolved, hash)) { releaseMedia(); return null; }
      }
      let settleContent, contentReleased=false; const contentPending = new Promise(resolve => { settleContent = resolve; });
      this.contentClaims.set(fingerprint, contentPending);
      const releaseContent = () => { if(contentReleased)return;contentReleased=true;if (this.contentClaims.get(fingerprint) === contentPending) this.contentClaims.delete(fingerprint); settleContent(); };
      return { row, resolved, mediaKey, file, stage, hash, verifiedSource, releaseMedia, releaseContent };
    } catch (error) {
      releaseSource();
      releaseMedia();
      throw error;
  ...
  async prepare(row, opts) {
    let retry = null;
    for (;;) {
      const job = await this.acquireSource(row, opts, retry);
      if (!job) return;
      let releaseExtraction;
      try {
        releaseExtraction = await opts.extractionGate.acquire(this.controller.signal);
        await this.extractAndPublish(job, opts);
        return;
      } catch (error) {
        const [alternate, ...remaining] = job.resolved.alternates || [];
        if (job.resolved.provider !== 'footagefarm' || !alternate || !M.isCorruptMediaError(error)) throw error;
        const failedUrl = job.resolved.url;
        const reclaimed = await ownedRemove(this.root, `.pipeline/media/${row.id}`, this.token)
          + await ownedRemove(this.root, `.pipeline/staging/${row.id}`, this.token);
        this.state.reclaimed += reclaimed;
        this.store.update(row.id, { source_hash: null, bytes: 0, error: null });
        const base = { ...job.resolved, alternates: [] }, next = typeof alternate === 'string' ? { ...base, url: alternate } : { ...base, ...alternate };
        retry = { mediaKey: job.mediaKey, resolved: { ...next, alternates: remaining } };
        await this.audit({ event: 'corrupt-rendition-retried', id: row.id, failed_media_url: failedUrl, retry_media_url: next.url, error: error.message, reclaimed_bytes: reclaimed });
      } finally {
        // A pause can abort while this source is waiting for an FFmpeg slot. It
        // must release its content/media claims even though extraction never ran.
        if (!releaseExtraction) { job.releaseContent?.(); job.releaseMedia?.(); }
        releaseExtraction?.();
      }
    }
  }
  async run(options = {}) {
    ...
    this.sourceClaims = new Map(); this.mediaClaims = new Map(); this.contentClaims = new Map(); this.resolvedAhead = null; this.extractingId = null; this.extractingIds = new Set();
    ...
    try {
      ...
      const limits=preparationLimits(this.getReviewLoad(),opts.maxGB,this.parallelism);
      const preparationConcurrency=limits.sourceConcurrency;
      const extractionGate=opts.extractionGate=concurrencyGate(limits.extractionConcurrency);
      ...
      this.update({preparationConcurrency,sourceConcurrency:preparationConcurrency,extractionConcurrency:limits.extractionConcurrency,message:`Preparing up to ${preparationConcurrency} sources with ${limits.extractionConcurrency} FFmpeg workers.`});
