...
async function get(url, signal, fetchImpl = fetch, accept = 'text/html') {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const response = await fetchImpl(url, { headers: { 'User-Agent': UA, Accept: accept }, signal: AbortSignal.any([signal || new AbortController().signal, AbortSignal.timeout(60000)]) });
...
async function download(resolved, folder, tools, { signal, onProgress, onCandidate, onRendition, maxBytes = 8 * 1024 ** 3 }) {
  ...
  for (let index = 0; index < candidates.length; index++) {
    ...
    try {
      if (candidate.provider === 'footagefarm-vimeo-lookup') {
        const fallback = await resolveFootageFarmVimeo({ reel: candidate.reel, duration: Number(candidate.duration) }, candidate.referer || candidate.url, signal, tools.fetch || fetch);
