...
const A = require('./public-archives.cjs');
...
async function resolveMedia(url, signal, fetchImpl = fetch) {
  ...
  const prelinger=A.prelingerId(url);
  if(prelinger){
    const metadata=await (await get(`https://archive.org/metadata/${encodeURIComponent(prelinger)}`,signal,fetchImpl,'application/json')).json(),candidates=A.archiveMp4s(metadata,prelinger);
    if(!candidates.length){const error=new Error('PREVIEW_UNAVAILABLE: This Prelinger item has no public MP4 movie file.');error.code='PREVIEW_UNAVAILABLE';throw error;}
    return{...candidates[0],alternates:candidates.slice(1)};
  }
  const loc=A.locItem(url);
  if(loc){
    const endpoint=`https://www.loc.gov/item/${encodeURIComponent(loc.id)}/?fo=json`,record=await (await get(endpoint,signal,fetchImpl,'application/json')).json(),videos=A.locVideos(record);
    if(!videos[loc.resource]){const error=new Error('PREVIEW_UNAVAILABLE: This Library of Congress item no longer exposes the selected MP4 resource.');error.code='PREVIEW_UNAVAILABLE';throw error;}
    return{provider:'loc-national-screening-room',url:videos[loc.resource],referer:`https://www.loc.gov/item/${encodeURIComponent(loc.id)}/`,direct:true,scope:'whole-reel'};
  }
