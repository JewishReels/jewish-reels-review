...
function createCheckpointSaver(cpPath, getCheckpoint, { atomicJson = S.atomicJson, sleep = R.sleep, delayMs = 100 } = {}) {
  let writes = Promise.resolve(), dirty = false, scheduled = false, revision = 0, savedRevision = 0, failure = null;
  const waiters = [];
  const settle = error => {
    for (let index = waiters.length - 1; index >= 0; index--) {
      const waiter = waiters[index];
      if (!error && waiter.revision > savedRevision) continue;
      waiters.splice(index, 1); error ? waiter.reject(error) : waiter.resolve();
    }
  };
  const schedule = delay => {
    if (scheduled || failure) return;
    scheduled = true;
    writes = writes.then(async () => {
      ...
      while (dirty) {
        ...
        const value = JSON.parse(JSON.stringify(getCheckpoint()));
        await atomicJson(cpPath, value);
        savedRevision = writingRevision;
        // Clear the flag before waking a waiter when this writer is drained.
        // The awakened video may immediately delete its checkpoint and request
        // another save; it must be able to schedule that next revision.
        if (!dirty) scheduled = false;
        settle();
        // Bound continuous 128-worker waves to one large checkpoint snapshot
        // per debounce interval instead of immediately rewriting it again.
        if (dirty && delayMs) await sleep(delayMs);
      }
    }).catch(error => { failure = error; scheduled = false; settle(error); });
  };
  return ({ wait = false } = {}) => {
    if (failure) return wait ? Promise.reject(failure) : Promise.resolve();
    dirty = true; const targetRevision = ++revision; schedule(wait ? 0 : delayMs);
    if (!wait || savedRevision >= targetRevision) return Promise.resolve();
    // Resolve after a snapshot containing this caller's changes. Later model
    // responses may remain dirty without holding a completed video open.
    return new Promise((resolve, reject) => waiters.push({ revision: targetRevision, resolve, reject }));
  };
}
...
class ReviewEngine extends EventEmitter {
  ...
  snapshot({includeDeferred=true}={}) {
    const value={...this.state,...this.clock?.snapshot(),busy:this.running};
  ...
  async run({ root, key, primary, secondary, verificationMode = 'all', detail = true, budget = 10, workers = 1, videoConcurrency = 1, dispatchMode = 'one-at-a-time', followPreparation = null, policy, frameFilter: filterSettings, sourceKey = null }) {
    ...
    if (!Number.isInteger(videoConcurrency) || videoConcurrency < 1 || videoConcurrency > 128) throw new Error('Choose between 1 and 128 videos at once.');
    ...
    this.running = true; this.pauseRequested = false; this.stopError = null; this.outputModes = new Map(); this.videoStates = new Map(); this.commitWrites = Promise.resolve(); this.commitBatch = []; this.commitBatchScheduled = false;
    // Workspace discovery and checkpoint restoration can take several seconds
    // for a large archive. Publish that transition before the first await so
    // the renderer cannot briefly re-enable Start and submit the same run twice.
    this.update({ status: 'starting', message: 'Opening the workspace and restoring saved review progress…' });
    const tasks = new Map(), committing = new Set(), settlingProgress = new Map();
    ...
    try {
      ...
      // The journal is workspace-wide so switching sources never erases pending
      // diagnostics. Only expose and retry rows that belong to this run's
      // source; merge the inactive rows back unchanged on every journal write.
      const belongsToSource = value => !sourceKey || (value?.source_key
        ? value.source_key === sourceKey
        : project.scopeIds?.has(String(value?.id)));
      const inactiveDeferred = new Map(savedDeferred.videos.filter(v => v?.id && !belongsToSource(v)).map(v => [String(v.id), v]));
      const deferred = new Map(savedDeferred.videos.filter(v => v?.id && belongsToSource(v) && !judged.has(String(v.id))).map(v => {
        ...
        return [String(v.id), { ...v, ...(sourceKey && !v.source_key ? { source_key: sourceKey } : {}),
      ...
      const saveDeferred = async () => {
        const active = [...deferred.values()];
        const videos = [...inactiveDeferred.values(), ...active];
        deferredWrites = deferredWrites.then(() => S.atomicJson(deferredPath, { version: 1, videos }));
        this.update({ deferred: active });
        await deferredWrites;
        this.emit('deferred-workspace', videos);
      };
      const defer = async issue => {
        ...
        const item = { ...issue, ...(sourceKey ? { source_key: sourceKey } : {}), attempts, next_retry_at: issue.manual ? null : Date.now() + delay };
      ...
      const globalCheckpointStore = { document: checkpoint, save: createCheckpointSaver(cpPath, () => checkpoint) };
      const checkpointStores = new Map();
      const checkpointStoreFor = async id => {
        if (!sourceKey) return globalCheckpointStore;
        // Keep large multi-source workspaces out of one ever-growing checkpoint.
        // Sixteen source-specific shards cap the bytes cloned and fsynced after
        // an image response while the legacy file remains a read-only fallback.
        const bucket = S.sha(String(id))[0];
        if (checkpointStores.has(bucket)) return checkpointStores.get(bucket);
        const folder = path.join(root, 'logs', 'reelsight-checkpoints', S.sha(sourceKey).slice(0, 16));
        const filename = path.join(folder, `${bucket}.json`);
        let document = await S.readJson(filename, null);
        if (document === null) {
          const videos = Object.fromEntries(Object.entries(checkpoint.videos).filter(([savedId]) =>
            S.sha(String(savedId))[0] === bucket && (!project.scopeIds || project.scopeIds.has(String(savedId)))));
          document = { version: 1, source_key: sourceKey, bucket, videos };
        }
        if (document.version !== 1 || document.source_key !== sourceKey || document.bucket !== bucket || !document.videos || typeof document.videos !== 'object') throw new Error('Unsupported source checkpoint shard. Keep the file for diagnosis.');
        const store = { document, save: null };
        store.save = createCheckpointSaver(filename, () => store.document);
        checkpointStores.set(bucket, store); return store;
      };
      ...
      const processVideo = async video => {
        ...
        const checkpointStore = await checkpointStoreFor(video.id);
        let cp = checkpointStore.document.videos[video.id];
        ...
        if (!cp || cp.configKey !== finalConfigKey || cp.fingerprint !== fingerprint || cp.cardSignature !== cardSignature) cp = { configKey: finalConfigKey, fingerprint, cardSignature, feedback: feedbackReference, completedCards: [], regions: {}, disagreements: 0 };
        checkpointStore.document.videos[video.id] = cp; checkpoint.videos[video.id] = cp;
        const saveCheckpoint = checkpointStore.save;
        ...
        // Model work for this video is complete.  A large workspace ledger can
        // take seconds to write, especially when several videos finish in the
        // same wave.  Keep that durable commit in `tasks`, but free the scarce
        // video-review slot now so another prepared clip can use the global
        // request pool while the verdict writer runs.
        settlingProgress.set(video.id, this.videoStates.get(video.id));
        committing.add(video.id); this.videoStates.delete(video.id); this.publishVideos();
        ...
        await this.commit(root, entry, judged, fingerprints);
        await resolved(video.id);
        delete checkpoint.videos[video.id]; delete checkpointStore.document.videos[video.id]; await saveCheckpoint({ wait: true });
      ...
      const launch = video => {
        ...
        const task = processVideo(video).catch(error => {
          ...
          const progress = this.videoStates.get(video.id) || settlingProgress.get(video.id);
          ...
          tasks.delete(video.id); committing.delete(video.id); settlingProgress.delete(video.id); this.videoStates.delete(video.id); this.publishVideos();
          this.emit('video-settled', video.id);
        });
        tasks.set(video.id, task);
      };
      let nextDiscovery = 0;
      const reviewsInProgress = () => tasks.size - committing.size;
      while (!this.pauseRequested && !this.stopError) {
        ...
        while (reviewsInProgress() < videoConcurrency && queue.length && !this.pauseRequested && !this.stopError) {
        ...
        if (reviewsInProgress() < videoConcurrency && !queue.length && (followPreparation || autoDeferred().length) && Date.now() >= nextDiscovery) {
  ...
  async commit(root, entry, judged, fingerprints) {
    const result = await new Promise((resolve, reject) => {
      this.commitBatch.push({ entry, resolve, reject });
      if (this.commitBatchScheduled) return;
      this.commitBatchScheduled = true;
      setTimeout(() => {
        this.commitBatchScheduled = false;
        const batch = this.commitBatch.splice(0);
        this.commitWrites = (this.commitWrites || Promise.resolve()).then(() => S.appendVerdicts(root, batch.map(item => item.entry)));
        this.commitWrites.then(results => results.forEach((value, index) => batch[index].resolve(value)), error => batch.forEach(item => item.reject(error)));
      }, 25);
    });
...
module.exports = { ReviewEngine, createCheckpointSaver };
