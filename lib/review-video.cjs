...
async function reviewVideo({ engine, root, video, cp, saveCheckpoint, key, primary, secondary, verificationMode, detail, workers, budget, runStartSpend, requestGate, feedbackContext }) {
  ...
  // A ReviewEngine worker setting is a global request ceiling, not a request
  // to create that many useful workers for every active video.  Starting 128
  // loops for each of 16 short videos produced 2,048 gate waiters.  After the
  // real regions were finished, those waiters acquired permits one by one only
  // to discover the generator was empty, leaving completed videos in the live
  // list and starving newly prepared work.
  //
  // Most prepared cards contain one region.  Keep some headroom for large
  // contact sheets while sizing each video's pool to the work it can expose.
  // A video can still use the entire global pool when it has enough cards.
  const remainingCards = video.cards.reduce((count, cardPath) => count + (cp.completedCards.includes(path.basename(cardPath)) ? 0 : 1), 0);
  const localWorkers = Math.min(workers, Math.max(1, remainingCards * 8));
  ...
  async function worker(workerId) {
    try {
      while (!stopped()) {
        ...
        const releaseJob = await requestGate.acquire(() => stopped() || sourceExhausted || budgetReached(), video.id);
        if (!releaseJob) return;
        workerId = releaseJob.workerId;
        let current;
        try {
        const next = await source.next();
        if (next.done) { sourceExhausted = true; return; }
        if (stopped()) return;
  ...
  await Promise.all(Array.from({ length: localWorkers }, (_, i) => worker(i + 1)));
  await source.return();
  publish();
