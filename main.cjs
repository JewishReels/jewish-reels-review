...
async function createWindow() {
  ...
  handle('add-website-source', async ({url}={}) => {
    ...
    const host=parsed.hostname.toLowerCase().replace(/^www\./,''),known=host==='myfootage.com'?{key:'myfootage',label:'MyFootage',homepage:'https://www.myfootage.com/'}:host==='footagefarm.com'?{key:'footagefarm',label:'Footage Farm',homepage:'https://footagefarm.com/'}:host==='archive.org'&&/^\/details\/prelinger\/?$/i.test(parsed.pathname)?{key:'prelinger',label:'Prelinger Archives',homepage:'https://archive.org/details/prelinger'}:host==='loc.gov'&&/^\/collections\/national-screening-room\/?$/i.test(parsed.pathname)?{key:'loc-national-screening-room',label:'Library of Congress · National Screening Room',homepage:'https://www.loc.gov/collections/national-screening-room/'}:null;
