...
let project = null, models = [], entries = [], selectedFilter = 'jewish', state = {}, connected = false, prefs = {}, currentEvidence = null, resultPage = 0;
...
function loadProject(p) {
  if (!p) return;
  if(matchCacheRoot!==p.root){matchCacheRoot=p.root;matchDetails.clear();matchImagePromises.clear();currentMatch=null;currentMatchId=null;} project = p; entries = p.entries || [];
  for(const [id,detail] of matchDetails){const summary=entries.find(entry=>String(entry.id)===id);if(!summary||(summary.feedback_target&&summary.feedback_target!==detail.feedback_target))matchDetails.delete(id);}
...
const acceptedMatches=()=>entries.filter(isAcceptedHit).slice().reverse();
...
function getResultDetail(value){
  if(value?.evidence)return Promise.resolve(value);
  const id=String(value?.id??value),summary=entries.find(e=>String(e.id)===id),saved=matchDetails.get(id);
  if(saved&&(!summary?.feedback_target||saved.feedback_target===summary.feedback_target)){matchDetails.delete(id);matchDetails.set(id,saved);return Promise.resolve(saved);}
  const pending=api.call('result-detail',id).then(detail=>{matchDetails.set(id,detail);trimCache(matchDetails,20);return detail;});
  return pending;
}
...
function renderPreparation(s){
  ...
  for(const id of ['vimeoAccessMode','vimeoProfile','chooseVimeoCookies','saveVimeoAccess'])$(id).disabled=['running','buffered','pausing','crawling'].includes(s.status);
