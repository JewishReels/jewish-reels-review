...
const websitePresets={footagefarm:{url:'https://footagefarm.com/',detail:'Footage Farm uses its tailored theme → subtheme → reel catalog adapter.'},myfootage:{url:'https://www.myfootage.com/',detail:'Add MyFootage without contacting the site. After permission is confirmed, the crawler reads the complete public video catalog through the site’s four official format partitions.'},prelinger:{url:'https://archive.org/details/prelinger',detail:'Prelinger Archives uses Internet Archive’s public cursor catalog and resolves each item to its public MP4 movie file.'},loc:{url:'https://www.loc.gov/collections/national-screening-room/',detail:'The Library of Congress adapter reads the National Screening Room JSON catalog and imports every public MP4 resource.'}};
...
$('crawlUrl').oninput=()=>{let host='',pathname='';try{const parsed=new URL($('crawlUrl').value);host=parsed.hostname.toLowerCase().replace(/^www\./,'');pathname=parsed.pathname;}catch{}const detected=host==='myfootage.com'?'myfootage':host==='footagefarm.com'?'footagefarm':host==='archive.org'&&/^\/details\/prelinger\/?$/i.test(pathname)?'prelinger':host==='loc.gov'&&/^\/collections\/national-screening-room\/?$/i.test(pathname)?'loc':'custom';if($('sourceWebsite').value!==detected){$('sourceWebsite').value=detected;renderWebsiteSource(false);}};
